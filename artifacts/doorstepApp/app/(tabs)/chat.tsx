...
function DiscoverTab({ insets }: { insets: any }) {
  const colors = useColors();
  const { discoverUsers, sendFriendRequest, hasPendingRequest, isFriend } = useChat();
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await discoverUsers(nextCursor);
      setUsers((prev) => {
        // filter out dupes
        const exist = new Set(prev.map(u => u.id));
        const newUsers = res.users.filter(u => !exist.has(u.id));
  ...
  return (
    <FlatList
      data={users}
      keyExtractor={(item) => item.id}
      contentContainerStyle={[styles.friendsContent, { paddingBottom: Platform.OS === "web" ? 34 : insets.bottom + 80 }]}
      showsVerticalScrollIndicator={false}
      onEndReached={handleLoadMore}
      onEndReachedThreshold={0.5}
      ListEmptyComponent={
        <View style={styles.friendsEmpty}>
          <Ionicons name="people-circle-outline" size={48} color={colors.mutedForeground} />
          <Text style={[styles.friendsEmptyText, { color: colors.mutedForeground, marginTop: 8 }]}>
      ...
      renderItem={({ item, index }) => {
        const pending = hasPendingRequest(item.id) || sentRequests.has(item.id);
        const alreadyFriend = isFriend(item.id);
        ...
        return (
          <Animated.View entering={FadeInDown.delay(Math.min(index, 10) * 40).springify()}>
            <View style={[styles.friendRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              >
                <UserAvatar displayName={item.displayName} avatarColor={item.avatarColor} size={52} />
              ...
              {alreadyFriend ? (
                <View style={[styles.friendBadge, { backgroundColor: colors.primary + "18" }]}>
                  <Text style={[styles.friendBadgeText, { color: colors.primary }]}>Friends</Text>
                </View>
              ) : pending ? (
                <View style={[styles.friendBadge, { backgroundColor: colors.muted }]}>
                  <Text style={[styles.friendBadgeText, { color: colors.mutedForeground }]}>Pending</Text>
                </View>
              ) : (
                <Pressable
                  ...
                  style={({ pressed }) => [
                    styles.addBtn,
                    { backgroundColor: pressed ? colors.primary + "E0" : colors.primary }
                  ]}
                >
                  <Ionicons name="person-add" size={14} color="#FFF" />
                  <Text style={styles.addBtnText}>Add</Text>
...
type TabKey = "chats" | "friends" | "discover";
...
export default function ChatScreen() {
  ...
  if (!currentUser) {
    // isLoaded but still no user — auth was cleared, router will redirect to login
    return (
      <View style={[styles.loadingRoot, { backgroundColor: colors.background }]}>
        <ActivityIndicator color={colors.primary} size="large" />
      </View>
  ...
  const getConvAvatarUrl = (convId: string) => {
    const conv = conversations.find((c) => c.id === convId);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.tabs, { borderBottomColor: colors.border }]}>
          {(["chats", "friends", "discover"] as TabKey[]).map((tab) => {
            const active = activeTab === tab;
            const label = tab === "chats" ? "Chats" : tab === "friends" ? "Friends" : "Discover";
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            const lastText = lastMsg
              ...
              : lastMsg.type === "contact" ? `Contact: ${lastMsg.contact?.name ?? "Shared contact"}`
              ...
              <Animated.View entering={FadeInDown.delay(index * 60).springify()}>
                ...
                >
                  <View style={{ position: "relative" }}>
                    <UserAvatar displayName={getConvName(item.id)} avatarColor={getConvColor(item.id)} avatarUrl={getConvAvatarUrl(item.id)} size={52} />
                    {/* Online dot for bots */}
                    {item.type === "direct" && (
          ...
          ListHeaderComponent={
            <View style={styles.storiesContainer}>
              <View style={styles.storiesHeader}>
                <Text style={[styles.storiesLabel, { color: colors.mutedForeground }]}>STATUS</Text>
              </View>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storiesScroll}>
                ...
                >
                  <View style={[
                    styles.storyRing,
                    { borderColor: activeStories[currentUser.id]?.length ? colors.primary : colors.border }
                  ]}>
                    <UserAvatar
                ...
                {/* Friend Stories — skip the current user's own entry */}
                {storyUsers
                  .filter((su) => su.userId !== currentUser.id)
                  .map((su, idx) => {
                    const displayName = su.user?.displayName ?? su.userId.slice(0, 8);
                    const avatarColor = su.user?.avatarColor ?? colors.primary;
                    const avatarUrl = su.user?.avatarUrl;
                    return (
                      <Animated.View key={su.userId} entering={FadeInRight.delay(idx * 50).springify()}>
                        ...
                        >
                          ...
                          <Text
                            style={[
                              styles.storyName,
                              { color: colors.foreground, fontFamily: su.hasUnseen ? "Inter_600SemiBold" : "Inter_400Regular" }
                            ]}
                            numberOfLines={1}
                          >
                            {displayName.split(" ")[0]}
      ...
      {activeTab === "discover" && <DiscoverTab insets={insets} />}
