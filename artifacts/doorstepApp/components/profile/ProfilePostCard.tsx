...
export const ProfilePostCard = memo(
  ({ post, isOwn, onDelete }: ProfilePostCardProps) => {
    ...
    // Audio Playback State will be managed by useAudioPlayer below
    ...
    const voicePlayer = useAudioPlayer(voiceUrl);

    // Determine the active player based on playing status, falling back to the appropriate player based on which url exists.
    const activePlayer = audioPlayer?.playing ? audioPlayer : voicePlayer?.playing ? voicePlayer : (audioPlayer || voicePlayer);
    const isPlaying = activePlayer?.playing ?? false;
    const position = activePlayer?.currentTime ?? 0;
    const duration = activePlayer?.duration ?? 1;

    const playPauseAudio = (url: string) => {
      const player = url === audioUrl ? audioPlayer : voicePlayer;
      if (!player) return;
      
      if (player.playing) {
        player.pause();
      } else {
        // Pause the other player just in case
        if (url === audioUrl) voicePlayer?.pause();
      ...
      >
        ...
        {imageUrl && (
          <View style={styles.imageWrapper}>
            ...
            {audioUrl && (
              ...
              >
                <Ionicons
                  name={isPlaying && activePlayer === audioPlayer ? "pause" : "musical-notes"}
                  size={14}
                  color="#fff"
                />
        ...
        {voiceUrl && (
          ...
          >
            ...
            >
              <Ionicons
                name={isPlaying && activePlayer === voicePlayer ? "pause" : "mic"}
                size={16}
                color="#fff"
              />
            </TouchableOpacity>
