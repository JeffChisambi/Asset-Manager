import { supabase, isDummyUrl } from "@/lib/supabase";
...
export class SupabasePostRepository implements IPostRepository {
  async createPost(
    ...
    if (isDummyUrl) {
      // Bypass upload/insert if offline — let fallback mechanisms cache it locally
      if (mediaFiles && mediaFiles.length) {
        for (const file of mediaFiles) {
          const prefix = file.name.startsWith("image") ? "image" : file.name.startsWith("music") ? "music" : "voice";
          mediaUrls.push(`${prefix}::${file.uri}`);
        }
      }
      return {
        id: `post_${Date.now()}`,
