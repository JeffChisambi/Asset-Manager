...
export default function CartScreen() {
  ...
  const haptic = useCallback((s: "light"|"medium"|"success") => {
    ...
    else if (s === "medium") Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
  ...
  useEffect(() => {
    ...
    const anim = Animated.loop(
      Animated.sequence([
        Animated.timing(bannerPulse, { toValue: 1.08, duration: 900, useNativeDriver: true }),
        Animated.timing(bannerPulse, { toValue: 1.0,  duration: 900, useNativeDriver: true }),
      ])
    );
    anim.start();
    return () => anim.stop();
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      ...
      Animated.timing(successOpacity, { toValue: 1, duration: 300, delay: 100, useNativeDriver: true }),
    ]).start();
    const t = setTimeout(() => {
      setStep("tracking");
  ...
  useEffect(() => {
    if (mainView === "history") {
      Animated.spring(historySlide, { toValue: 0, friction: 8, useNativeDriver: true }).start();
    } else {
  ...
  function resumeTracking() {
    ...
    setStep("tracking");
    haptic("medium");
    Animated.spring(panelSlide, { toValue: 1, friction: 7, delay: 100, useNativeDriver: true }).start();
  }
    ...
    return (
      <View style={[s.screen, { backgroundColor: colors.background, paddingTop: topPad }]}>
        ...
        {allOrders.length === 0 ? (
          ...
          >
            {allOrders.map((order) => {
              const isDelivered = order.finalStage >= DELIVERY_STAGES.length - 1;
              const statusLabel = order.isActive
                ? DELIVERY_STAGES[order.finalStage]?.label ?? "In Transit"
                : isDelivered ? "Delivered" : "Incomplete";
              const statusColor = order.isActive
                ? colors.primary
                : isDelivered ? "#22C55E" : colors.mutedForeground;
              const statusBg = order.isActive
                ? colors.primary + "15"
                : isDelivered ? "#22C55E18" : colors.muted;
                ...
                >
                  ...
                  <View style={s.historyTop}>
                    <View style={s.historyLeft}>
                      ...
                      <Text style={[s.historyOrderId, { color: colors.foreground }]}>
                        {order.orderId}
                      </Text>
                    </View>
                    <Text style={[s.historyTotal, { color: colors.foreground }]}>
                  ...
                  <View style={s.historyMeta}>
                    <View style={[s.historyStatusBadge, { backgroundColor: statusBg }]}>
                      {order.isActive && (
                        <View style={[s.liveDotSmall, { backgroundColor: colors.primary }]} />
                      )}
                      <Text style={[s.historyStatusTxt, { color: statusColor }]}>
                        {statusLabel}
                      </Text>
                    </View>
                  ...
                  <View style={s.historyItems}>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                      {order.items.slice(0, 4).map((item) => (
                        <View key={item.id} style={[s.historyThumb, { backgroundColor: colors.muted }]}>
                          {item.imageUrl ? (
                            <Image source={{ uri: item.imageUrl }} style={s.historyThumbImg} />
                          ) : item.imageSource ? (
                            <Image source={item.imageSource} style={s.historyThumbImg} />
                          ) : (
                      ...
                      {order.items.length > 4 && (
                        <View style={[s.historyThumb, s.historyMore, { backgroundColor: colors.primary + "20" }]}>
                          <Text style={[s.historyMoreTxt, { color: colors.primary }]}>
                            +{order.items.length - 4}
                          </Text>
                        </View>
                      )}
                  ...
                  {/* Footer */}
                  <View style={[s.historyFooter, { borderTopColor: colors.border }]}>
                    <View style={s.historyFooterLeft}>
                      <Ionicons name="card-outline" size={13} color={colors.mutedForeground} />
                      <Text style={[s.historyPayment, { color: colors.mutedForeground }]}>
                        {order.paymentName}
                      </Text>
                      <Text style={[s.historyPayment, { color: colors.border }]}>·</Text>
                      <Text style={[s.historyPayment, { color: colors.mutedForeground }]}>
                        {order.items.reduce((s, i) => s + i.quantity, 0)} item{order.items.length !== 1 ? "s" : ""}
                      </Text>
                    </View>
                    {order.isActive && (
                      <View style={[s.trackCta, { backgroundColor: colors.primary }]}>
    ...
    return (
      <View style={[s.screen, { backgroundColor: colors.background, paddingTop: topPad }]}>
        <View style={s.header}>
          ...
          <Pressable
            onPress={() => { setMainView("history"); haptic("light"); }}
            style={[s.historyTabBtn, { backgroundColor: colors.muted }]}
          >
            <Ionicons name="receipt-outline" size={16} color={colors.foreground} />
            <Text style={[s.historyTabTxt, { color: colors.foreground }]}>Orders</Text>
            {(orderHistory.length > 0 || activeOrder) && (
              <View style={[s.ordersBadge, { backgroundColor: colors.primary }]}>
    ...
    return (
      <View style={[s.screen, { backgroundColor: colors.background, paddingTop: topPad }]}>
        <View style={s.header}>
          ...
          <View style={s.headerRight}>
            ...
            <Pressable onPress={() => { haptic("medium"); clearCart(); }}>
              <Text style={[s.clearBtn, { color: colors.mutedForeground }]}>Clear</Text>
            </Pressable>
          </View>
        </View>
              ...
              <View style={s.cartInfo}>
                ...
                <View style={s.cartBottom}>
                  ...
                  <View style={[s.qtyRow, { backgroundColor: colors.muted }]}>
                    <Pressable style={s.qtyBtn} onPress={() => updateQuantity(item.id, item.quantity - 1)}>
        ...
        <View style={[s.sticky, { backgroundColor: colors.card, borderTopColor: colors.border, paddingBottom: insets.bottom + 12 }]}>
          ...
          <Pressable style={[s.checkoutBtn, { backgroundColor: colors.primary }]} onPress={() => navigateTo("details")}>
    ...
    return (
      <KeyboardAvoidingView style={[s.screen, { backgroundColor: colors.background }]} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <Animated.View style={[s.stepWrap, { paddingTop: topPad + 12, transform: [{ translateY: slideAnim }] }]}>
          <StepHeader title="Delivery Details" subtitle="Where should we send your order?" step={1} totalSteps={2} onBack={() => navigateTo("cart")} colors={colors} />
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.stepScroll}>
            ...
            <View style={[s.segmented, { backgroundColor: colors.muted, marginBottom: 20 }]}>
              {(["Pickup", "Delivery"] as DeliveryType[]).map(dt => (
                <Pressable key={dt} style={[s.seg, deliveryType === dt && { backgroundColor: colors.primary }]} onPress={() => { setDeliveryType(dt); haptic("light"); }}>
                  <Ionicons name={dt === "Pickup" ? "walk-outline" : "bicycle-outline"} size={14} color={deliveryType === dt ? "#fff" : colors.mutedForeground} />
                  <Text style={[s.segTxt, { color: deliveryType === dt ? "#fff" : colors.mutedForeground }]}>
    ...
    return (
      <View style={[s.screen, { backgroundColor: colors.background }]}>
        <Animated.View style={[s.stepWrap, { paddingTop: topPad + 12, flex: 1, transform: [{ translateY: slideAnim }] }]}>
          <StepHeader title="Payment" subtitle="Secure & fast checkout" step={2} totalSteps={2} onBack={() => navigateTo("details")} colors={colors} />
          <PaymentStep selected={paymentId} onSelect={id => { setPaymentId(id); haptic("light"); }} colors={colors} total={subtotal} deliveryFee={fee} />
        </Animated.View>
        <View style={[s.stepFooter, { borderTopColor: colors.border, paddingBottom: insets.bottom + 12 }]}>
          <Pressable style={[s.checkoutBtn, { backgroundColor: paymentId ? colors.primary : colors.muted }]} disabled={!paymentId} onPress={handleCheckout}>
      ...
      <View style={[s.screen, s.successScreen, { backgroundColor: colors.background, paddingTop: topPad }]}>
        ...
        <Animated.View style={[{ opacity: successOpacity, alignItems: "center", width: "100%" }]}>
          ...
          <View style={[s.successCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={s.successRow}><Text style={[s.successRowLabel, { color: colors.mutedForeground }]}>Order ID</Text><Text style={[s.successRowVal, { color: colors.primary }]}>{currentOrderId}</Text></View>
            <View style={[s.divider, { backgroundColor: colors.border }]} />
            <View style={s.successRow}><Text style={[s.successRowLabel, { color: colors.mutedForeground }]}>Items</Text><Text style={[s.successRowVal, { color: colors.foreground }]}>{snapshotItems.reduce((acc, i) => acc + i.quantity, 0)} items</Text></View>
            <View style={[s.divider, { backgroundColor: colors.border }]} />
            <View style={s.successRow}><Text style={[s.successRowLabel, { color: colors.mutedForeground }]}>Payment</Text><Text style={[s.successRowVal, { color: colors.foreground }]}>{PAYMENT_METHODS.find(m => m.id === paymentId)?.name ?? "—"}</Text></View>
            <View style={[s.divider, { backgroundColor: colors.border }]} />
            <View style={s.successRow}><Text style={[s.successRowLabel, { color: colors.mutedForeground }]}>Total Paid</Text><Text style={[s.successRowVal, { color: colors.foreground, fontFamily: "Inter_800ExtraBold" }]}>${total.toFixed(2)}</Text></View>
          </View>
          <Text style={[s.successHint, { color: colors.mutedForeground }]}>Taking you to live tracking...</Text>
        </Animated.View>
      </View>
    );
  }
  ...
  const currentStage  = DELIVERY_STAGES[liveStage] ?? DELIVERY_STAGES[DELIVERY_STAGES.length - 1];
    ...
    <View style={s.screen}>
      ...
      <View style={[s.topBar, { paddingTop: topPad + 8 }]}>
        <Pressable
          ...
          onPress={() => { setStep("cart"); panelSlide.setValue(0); }}
        >
        ...
        <View style={[s.orderIdPill, { backgroundColor: colors.card + "EE" }]}>
          ...
          <Text style={[s.orderIdTxt, { color: colors.foreground }]}>
            {isDelivered ? "✓ Delivered" : (trackingOrder?.orderId ?? currentOrderId)}
          </Text>
        </View>
        <Pressable
          style={[s.topBackBtn, { backgroundColor: colors.card + "EE", borderColor: colors.border }]}
          onPress={() => { setMainView("history"); setStep("cart"); panelSlide.setValue(0); haptic("light"); }}
        >
      ...
      <View style={s.driverFab}>
        <Pressable style={[s.fabBtn, { backgroundColor: colors.primary }]} onPress={() => haptic("medium")}>
          <Ionicons name="call-outline" size={20} color="#fff" />
        </Pressable>
      </View>

      {/* Bottom panel */}
      <Animated.View style={[s.panel, { backgroundColor: colors.card, transform: [{ translateY: panelTranslate }] }]}>
        ...
        <View style={s.driverRow}>
          <View style={[s.driverAvatar, { backgroundColor: colors.primary }]}>
            <Text style={s.driverInitials}>{trackingOrder?.driverInitials ?? driver.initials}</Text>
          </View>
          <View style={s.driverInfo}>
            <Text style={[s.driverName, { color: colors.foreground }]}>{trackingOrder?.driverName ?? driver.name}</Text>
          ...
          <View style={[s.etaBadge, { backgroundColor: colors.primary + "15" }]}>
            <Text style={[s.etaNum, { color: colors.primary }]}>{isDelivered ? "0" : liveEta}</Text>
        ...
        <View style={[s.stageBanner, { backgroundColor: isDelivered ? "#22C55E18" : colors.primary + "12", borderColor: isDelivered ? "#22C55E30" : colors.primary + "30" }]}>
          <Ionicons name={currentStage.icon} size={16} color={isDelivered ? "#22C55E" : colors.primary} />
          <Text style={[s.stageTxt, { color: isDelivered ? "#22C55E" : colors.primary }]}>{currentStage.label}</Text>
        ...
        {/* Timeline */}
        <ScrollView style={s.timelineScroll} showsVerticalScrollIndicator={false} scrollEnabled={false}>
          <DeliveryTimeline currentStage={liveStage} colors={colors} compact />
        </ScrollView>

        {/* Items scroll */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.itemsScroll}>
          {(trackingOrder?.items ?? snapshotItems).map((item: any) => (
            <View key={item.id} style={[s.trackCard, { backgroundColor: colors.background, borderColor: colors.border }]}>
              <View style={[s.trackImgWrap, { backgroundColor: colors.muted }]}>
                {item.imageUrl ? <Image source={{ uri: item.imageUrl }} style={s.trackImg} />
                  : item.imageSource ? <Image source={item.imageSource} style={s.trackImg} />
...
function ActiveOrderBanner({
  order, liveStage, liveEta, colors, pulseAnim, onPress,
}: {
  order: any; liveStage: number; liveEta: number;
  colors: any; pulseAnim: Animated.Value; onPress: () => void;
}) {
    ...
    <Pressable onPress={onPress} style={styles.bannerOuter}>
      ...
      >
        <View style={styles.bannerLeft}>
          <Animated.View style={[styles.bannerIconWrap, { transform: [{ scale: pulseAnim }] }]}>
          ...
          <View style={styles.bannerText}>
            ...
            <Text style={styles.bannerStage} numberOfLines={1}>{stage.label}</Text>
...
const styles = StyleSheet.create({
  bannerOuter: { marginHorizontal: 16, marginBottom: 12, borderRadius: 18, overflow: "hidden" },
  bannerGrad: { flexDirection: "row", alignItems: "center", padding: 14, gap: 12 },
  bannerLeft: { flex: 1, flexDirection: "row", alignItems: "center", gap: 12 },
  bannerIconWrap: { width: 44, height: 44, borderRadius: 14, backgroundColor: "rgba(255,255,255,0.2)", alignItems: "center", justifyContent: "center" },
  bannerText: { flex: 1 },
  bannerTopRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 2 },
  bannerLiveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#FF4444" },
  bannerLiveLabel: { fontSize: 10, fontFamily: "Inter_700Bold", color: "rgba(255,255,255,0.8)", letterSpacing: 1 },
  bannerOrderId: { fontSize: 11, fontFamily: "Inter_600SemiBold", color: "rgba(255,255,255,0.7)" },
  bannerStage: { fontSize: 14, fontFamily: "Inter_700Bold", color: "#fff", marginBottom: 1 },
  bannerEta: { fontSize: 12, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.8)" },
  bannerCta: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "rgba(255,255,255,0.2)", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12 },
...
const s = StyleSheet.create({
  ...
  historyTabBtn: { flexDirection:"row", alignItems:"center", gap:6, paddingHorizontal:12, paddingVertical:7, borderRadius:12 },
  historyTabTxt: { fontSize:13, fontFamily:"Inter_600SemiBold" },
  ordersBadge: { width:18, height:18, borderRadius:9, alignItems:"center", justifyContent:"center" },
  ordersBadgeNum: { fontSize:10, fontFamily:"Inter_700Bold", color:"#fff" },
  historyBackBtn: { width:38, height:38, borderRadius:12, alignItems:"center", justifyContent:"center" },
  // ── Empty ──
  ...
  // ── History ──
  historyList: { paddingHorizontal:16, paddingTop:4, paddingBottom:40, gap:14 },
  historyCard: { borderRadius:20, borderWidth:1, overflow:"hidden" },
  historyTop: { flexDirection:"row", justifyContent:"space-between", alignItems:"center", padding:16, paddingBottom:10 },
  ...
  historyMeta: { flexDirection:"row", alignItems:"center", gap:10, paddingHorizontal:16, paddingBottom:12 },
  historyStatusBadge: { flexDirection:"row", alignItems:"center", gap:5, paddingHorizontal:10, paddingVertical:4, borderRadius:20 },
  ...
  historyThumb: { width:50, height:50, borderRadius:12, alignItems:"center", justifyContent:"center", overflow:"hidden" },
  ...
  historyFooter: { flexDirection:"row", justifyContent:"space-between", alignItems:"center", borderTopWidth:1, paddingHorizontal:16, paddingVertical:12 },
  ...
  topBackBtn: { width:44, height:44, borderRadius:13, borderWidth:1, alignItems:"center", justifyContent:"center", shadowColor:"#000", shadowOpacity:0.12, shadowRadius:8, shadowOffset:{width:0,height:3}, elevation:5 },
  orderIdPill: { flexDirection:"row", alignItems:"center", gap:7, paddingHorizontal:16, paddingVertical:10, borderRadius:20, shadowColor:"#000", shadowOpacity:0.1, shadowRadius:6, shadowOffset:{width:0,height:2}, elevation:4 },
  ...
  stageBanner: { flexDirection:"row", alignItems:"center", gap:8, marginHorizontal:20, marginBottom:12, paddingHorizontal:14, paddingVertical:10, borderRadius:14, borderWidth:1 },
